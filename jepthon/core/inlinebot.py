...
def main_menu():
    text = f"**- [سـورس القرش](https://t.me/L_H_V)\
        \n المسـاعد\
        \n\
        \n◽ القرش لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            buttons = [
                (
                    ...
                    Button.url(" 𝚂𝙷𝙰𝚁𝙺 UsᴇʀBoᴛ", "https://t.me/L_H_V"),
                )
            ]
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس القرش 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس القرش 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت القرش 🧸♥](https://t.me/L_H_V)**\
            \n\
            \n❤ القرش هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/QW_PN)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
